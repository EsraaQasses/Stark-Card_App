...
const FullPayments = () => {
  ...
  // Filter and transform data for grid
  const filteredData = useMemo(() => {
    ...
    if (filters.endDate) {
      filtered = filtered.filter(payment => 
        new Date(payment.created_at) <= new Date(filters.endDate + 'T23:59:59')
      );
    }
    ...
    return (
      <div className="flex items-center gap-2">
        <div className="w-8 h-8 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-xs">
          {props.UserName?.split(' ').map(n => n[0]).join('').substring(0, 2) || 'US'}
        </div>
        <div>
  ...
  const handleViewDetails = (payment) => {
    ...
    alert(`Payment Details:\nID: ${payment.PaymentID}\nUser: ${payment.UserName}\nProduct: ${payment.ProductName}\nAmount: ${payment.Currency === 'USD' ? '$' : 'SYP '}${payment.FinalPrice}\nStatus: ${payment.Status}`);
  };
