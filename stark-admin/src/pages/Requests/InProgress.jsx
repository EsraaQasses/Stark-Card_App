...
const InProgress = () => {
  ...
  // Fetch in-progress requests from backend
  useEffect(() => {
  ...
  const fetchInProgressRequests = async () => {
    try {
      setLoading(true);
      setError(null);
      // Fetch requests with in_progress status
  ...
  const calculateStats = (requests) => {
    ...
    const stats = {
      ...
      underReview: requests.filter(item => 
  ...
  const handleApprovePayment = async (requestId, customerName, amount, currency) => {
    if (window.confirm(`Approve payment of ${amount} ${currency?.toUpperCase()} from ${customerName}?`)) {
      try {
        ...
        fetchInProgressRequests(); // Refresh data
      } catch (error) {
...
Comments: ${requestDetails.comments?.length || 0}
  ...
  const statusTemplate = (props) => {
    ...
    const config = statusConfig[request.status] || { color: 'bg-gray-500', icon: '❓', label: request.status, progress: 0 };
  ...
  const requestTypeTemplate = (props) => {
    ...
    const config = typeConfig[request.request_type] || { color: 'bg-gray-100 text-gray-800', label: request.request_type };
  ...
  const actionTemplate = (props) => {
    ...
    const canEscalate = request.amount > 500; // Escalate high amount requests
