...
const Blacklist = () => {
  ...
  // Fetch banned users from backend
  ...
  const toolbarClick = async (args) => {
    if (args.item.id.includes('deletegrid')) {
      ...
      if (selected.length > 0) {
        if (window.confirm(`Are you sure you want to permanently delete ${selected.length} user(s)?`)) {
          try {
            // Implement permanent deletion if needed
    ...
    if (args.item.id.includes('Unblock')) {
      ...
      if (selected.length > 0) {
        try {
          const unbanPromises = selected.map(user => 
            axiosInstance.post(`/users/unban/${user.id}/`)
          );
          
          await Promise.all(unbanPromises);
          await fetchBannedUsers(); // Refresh data
  ...
  // Blacklist grid columns configuration
  const blacklistGrid = [
    { 
      type: 'checkbox', 
      width: '50' 
